...
public class RemoteConfig : MonoBehaviour
{
    ...
    [SerializeField] private float _collapsibleBannerInterval = 20f;
    [SerializeField] private float _timeBetweenAds = 60f;
    ...
    public float FREE_REWARD_INTERVAL => _freeRewardInterval;
    ...
    public void InitializeRemoteConfig()
    {
        ...
        var defaults = new Dictionary<string, object>
        {
            ...
            { "free_reward_interval", _freeRewardInterval},
        };
    ...
    private void FetchRemoteConfig()
    {
        FirebaseRemoteConfig.DefaultInstance.FetchAndActivateAsync().ContinueWithOnMainThread(task =>
        {
            if (task.IsCompleted && !task.IsFaulted && !task.IsCanceled)
            {
                ...
                var freeRewardInterval  = FirebaseRemoteConfig.DefaultInstance.GetValue("free_reward_interval").DoubleValue;
                if (_showDebug)
                {
                ...
                _freeRewardInterval = (float)freeRewardInterval;
                fetch = true;
            }
            else
            {
